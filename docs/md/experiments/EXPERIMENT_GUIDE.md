...
| 목적 | config |
| --- | --- |
| embedding 모델 교체 후보 | `rag.embedding.provider: huggingface` |
| reranker 확장 후보 | `rag.reranker.provider: huggingface` |
| LLM 답변 생성 참고 후보 | `rag.answerer.provider: huggingface` |

HuggingFace LangChain integration은 현재 기본 `requirements.txt`에 포함하지 않습니다. `transformers` 계열 버전과 충돌할 수 있으므로, 실제로 사용할 때는 별도 호환 환경에서 먼저 검증합니다.
