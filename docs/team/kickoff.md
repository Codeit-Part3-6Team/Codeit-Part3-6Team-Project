...
이번 프로젝트의 1차 목표는 최종 앱을 바로 만드는 것이 아닙니다. 팀이 공통으로 사용할 수 있는 config 기반 RAG 실험 파이프라인과 산출물 규칙을 안정적으로 준비하는 것입니다.
...
로컬 실행과 Colab 실행을 모두 고려합니다.

- 로컬: 빠른 수정, smoke test, 작은 샘플 실험
- Colab: 팀원 PC 자원이 부족하거나 장시간 실행이 필요할 때
...
GitHub repository는 코드, config, 문서, Issue, PR, 실험 기록의 기준 위치입니다. 노트북이나 Colab을 쓰더라도 결과와 결정은 GitHub에 남깁니다.
...
| 로컬 자원 차이가 큼 | 팀원별 실행 가능 범위가 달라짐 | Colab 실행 노트북과 작은 smoke config를 병행합니다. |
...
- [ ] 로컬 또는 Colab에서 RAG 실행 흐름을 한 번 확인합니다.
