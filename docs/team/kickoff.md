...
이번 프로젝트의 1차 목표는 config 기반 RAG 실험 파이프라인과 산출물 규칙을 안정적으로 준비하고, 실험으로 검증된 최적 설정을 바탕으로 RFP 문서 분석 서비스를 완성하는 것입니다. 추가로 Agent Loop(Phase DAG + Tool dispatch + Structured Output + Chatbot)를 config 기반으로 통합하여 요약/비교/추천 기능을 제공합니다.
...
### Agent 모드 (Phase DAG, Tool dispatch, Structured Output, Chatbot)

RAG 파이프라인의 다음 단계로 Agent 모드를 추가합니다. Agent는 Phase DAG 기반으로 일정 단계가 아닌 분기/조합 단계로 동작하며, Tool dispatch를 통해 문서 검색, 비교, 구조화된 판단을 수행합니다. Structured Output은 답변을 규격화된 JSON 구조로 남기고, Chatbot 모드로 대화형 RFP 분석을 지원합니다.
