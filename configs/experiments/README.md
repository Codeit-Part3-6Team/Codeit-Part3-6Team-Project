...
- `rag/rag_langchain.yaml`: LangChain 엔진 기반 기본 RAG 실험
- `rag/rag_semantic.yaml`: local semantic retriever 비교 실험
- `rag/rag_keyword.yaml`: keyword retriever 비교 실험
- `rag/rag_hybrid.yaml`: keyword + semantic hybrid 비교 실험

새 RAG 실험을 만들 때는 기존 config를 복사한 뒤 최소한 `experiment.name`, `paths.output_dir`, `rag.retriever`, `rag.splitter` 또는 `rag.chunk`, `artifact_policy.run_id`를 확인합니다.
